package com.bupt.ZigbeeResolution.aop;
...
import com.bupt.ZigbeeResolution.service.ILoggingEventService;
...
public class LogAop {
	...
	@Pointcut("execution( * com.bupt.ZigbeeResolution.controller.*.*(..))")
	public void log() {
	}
...
//					}
					...
					loggingEvent.setFormatted_message("ip地址为" + request.getRemoteHost() + " "+ user +  " 执行了 " + controllerOperation + " 下的  " + methodOperation + " 操作！");
	...
	public void afterReturing(JoinPoint joinPoint) {
		...
		if(user != null){
			logger.info(user.getUserName() + " 执行了 " + controllerOperation + " 下的  " + methodOperation + " 操作！ ip地址为"
	...
	public void afterThrowing(JoinPoint joinPoint, Throwable e) {
		...
		User user = (User) request.getSession().getAttribute("user");
		logger.info(user.getUserName() + " 执行了 " + controllerOperation + " 下的  " + methodOperation + " 操作 出现了异常！异常信息："+e.getMessage()+ "   ip地址为"
				+ request.getRemoteAddr());
		System.err.println(user.getUserName() + " 执行了 " + controllerOperation + " 下的  " + methodOperation + " 操作 出现了异常！ 异常信息："+e.getMessage()+"ip地址为"
				+ request.getRemoteAddr());
	}
}
